...
const ServiceOverview: React.FC = () => {
    ...
    >
      ...
      >
        ...
        <div className="text-text-primary flex flex-col gap-2 px-6 py-3 text-center">
          ...
          <p className="text-16px font-normal whitespace-pre-line text-text-secondary">{t(`features.${feature.key}.description`)}</p>
        </div>
      </div>
    </FadeIn>
  ));
  ...
  return (
    <div className="relative z-10 flex flex-col items-center justify-center gap-8 py-10">
      ...
      <FadeIn className="z-10 flex flex-col items-center gap-1 font-bold">
      ...
      <div className="z-10 grid grid-cols-1 gap-8 px-4 md:grid-cols-2 lg:grid-cols-3 lg:gap-[40px] lg:px-8 max-w-[1440px]">
        {serviceTickets}
      </div>
    </div>
  );
